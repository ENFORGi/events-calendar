...
func (r *UserEventsRepositoriy) FindByIdUserEvent(id uint) (*generate_models.Usersevent, error) {
	var userEvent generate_models.Usersevent
	err := r.db.First(&userEvent, id).Error
	return &userEvent, err
}

func (r *UserEventsRepositoriy) GetUserEvents(userID uint) ([]*generate_models.Event, error) {
